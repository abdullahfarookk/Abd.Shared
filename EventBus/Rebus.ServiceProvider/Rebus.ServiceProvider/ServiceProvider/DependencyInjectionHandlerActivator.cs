...
public class DependencyInjectionHandlerActivator : IHandlerActivator
{
    ...
    IReadOnlyList<IEventHandler<TMessage>> GetMessageHandlersForMessage<TMessage>(IServiceScope scope)
    {
        ...
        return typesToResolve
            .SelectMany(type => serviceProvider.GetServices(type).Cast<IEventHandler>())
            .Distinct(new TypeEqualityComparer())
            .Cast<IEventHandler<TMessage>>()
            .ToList();
    }
    ...
    static Type[] FigureOutTypesToResolve(Type messageType)
    {
        ...
        return handledMessageTypes
            .Select(t => typeof(IEventHandler<>).MakeGenericType(t))
            .ToArray();
    }
